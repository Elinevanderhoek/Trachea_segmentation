...
def crop_image(image):
    ...
    # ax[0].imshow(gray, cmap='Greys')
    # ax[0].set_title('Original Image')
    # ax[1].imshow(mask_cropped, cmap='Greys')
    # ax[1].set_title('Mask cropped')
    # ax[2].imshow(img_cropped)
    # ax[2].set_title('Img croppped')
    # plt.show()

    # mask_cropped = (mask_cropped).clip(0, 255).astype(np.uint8)
    # img_cropped = (img_cropped).clip(0, 255).astype(np.uint8)
...
def post_process_prediction(pred_mask, kernel_size, iterations):
    ...
    #pred_mask = skimage.exposure.equalize_adapthist(pred_mask)
    # Apply threshold
    binary_mask = (pred_mask.squeeze() > threshold).astype(np.uint8)
    #filled_mask = ndimage.binary_fill_holes(binary_mask).astype(np.uint8)
    # # Apply Gaussian blur
    # blurred_mask = cv2.GaussianBlur(binary_mask, gaussian_kernel_size, gaussian_sigma)

    # # Re-apply threshold after blur
    # binary_mask = (blurred_mask > 0.5).astype(np.uint8)
    ...
    # # Get the largest non-border contour
    # height, width = binary_mask.shape
    # final_pred_mask = fill_biggest_non_border_contour(cnts, width, height)
